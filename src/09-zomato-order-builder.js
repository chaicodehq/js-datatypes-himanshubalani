...
*   // subtotal: 330, deliveryF ee: 30, gst: 16.5, discount: 100
...
export function buildZomatoOrder(cart, coupon) {
  ...
  addonTotal = item.addons
    .map(addon => Number(addon.split(":")[1]))  // Extract price from "Name:Price"
    .reduce((sum, price) => sum + price, 0);
}
    let itemTotal = (basePrice + addonTotal) * item.qty
    return { name , basePrice, qty, addonTotal, itemTotal}
  }
  )
  let subtotalx = items.reduce((sum, item) => sum + item.itemTotal, 0);
  let deliveryFeex = 30

  if (subtotalx < 500) deliveryFeex = 30
  if (subtotalx >= 500 && subtotalx < 1000) deliveryFeex = 15
  if (subtotalx >= 1000) deliveryFeex = 0

  let gstx = parseFloat((0.05 * subtotalx).toFixed(2));
  let grandTotalx = 0;
  let discountx;
  let couponx;

  if (typeof coupon === "string") {couponx = coupon.toLowerCase()}
  else {couponx = "invaild"}
  
  if (couponx == 'flat100') {
    discountx = 100
  } else if (couponx === 'first50') {
    discountx = Math.min((subtotalx * 0.5), 150);
  } else if (couponx === 'freeship') {
    discountx = deliveryFeex;
    deliveryFeex = 0
  } else {
    discountx = 0
  }

  grandTotalx = subtotalx + gstx + deliveryFeex - discountx;
   if (grandTotalx <= 0) grandTotalx = 0;

  return { items: items, subtotal: subtotalx, deliveryFee: deliveryFeex, gst: gstx, discount: discountx, grandTotal: grandTotalx
}
}
